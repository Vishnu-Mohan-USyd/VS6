...
class RgcLgnV1Network:
    ...
    def rgc_drive_grating(self, theta_deg: float, t_ms: float, phase: float, *,
        ...
        dog_pad = np.fft.irfft2(np.fft.rfft2(stim_pad) * self._rgc_dog_fft, s=stim_pad.shape).astype(np.float32, copy=False)
...
def main() -> None:
    ...
    theta_offset = 0.0
    theta_step = 0.0
    if args.train_theta_schedule == "low_discrepancy":
        phi = (1.0 + math.sqrt(5.0)) / 2.0
        theta_step = 180.0 / phi
