...
def wilcoxon_signed_rank_greater(x: np.ndarray) -> float:
    """One-sided Wilcoxon signed-rank test p-value for median(x) > 0.

    Falls back to a normal approximation when SciPy isn't available.

    Parameters
    ----------
    x : ndarray
        Paired differences (1D). Zeros are ignored. NaNs are dropped.

    Returns
    -------
    p : float
        One-sided p-value for alternative "greater" (median > 0).
    """
    ...
    # Try SciPy if available (exact/tie-aware); otherwise use approximation.
    try:  # pragma: no cover
        from scipy.stats import wilcoxon as _wilcoxon  # type: ignore

        _stat, p = _wilcoxon(x, alternative="greater")
        return float(p)
    except ModuleNotFoundError:
        pass

    absx = np.abs(x)
    order = np.argsort(absx, kind="mergesort")  # stable for ties
    ...
    while i < n:
        j = i
        # Exact tie handling (rare for float data, but correct when it happens)
        while (j + 1) < n and absx[order[j + 1]] == absx[order[i]]:
            j += 1
        # Average rank for tied block [i, j]
        avg_rank = 0.5 * ((i + 1) + (j + 1))
        ranks[order[i : j + 1]] = avg_rank
        t = (j - i + 1)
        if t > 1:
            tie_correction += float(t * t * t - t)
        i = j + 1

    w_plus = float(ranks[x > 0.0].sum())
    mean = float(n * (n + 1) / 4.0)
    var = float((n * (n + 1) * (2 * n + 1) - tie_correction) / 24.0)
    if var <= 1e-12:
        return 1.0

    # Continuity correction for one-sided "greater"
    z = (w_plus - mean - 0.5) / math.sqrt(var)
    # Survival function of standard normal using erfc
...
class Params:
    ...
    # Thalamocortical retinotopy heterogeneity.
    # Real V1 neurons within a local neighborhood do not share an identical RF center; small center scatter
    # reduces pathological "exchangeability" where many neurons converge to the same feature under purely
    # feedforward Hebbian learning. Implemented by shifting the center of the retinotopic weight caps and
    # structural sparsity sampling per postsynaptic unit.
    tc_center_jitter: float = 0.0  # pixels (0 disables)
    ...
    pv_in_sigma: float = 1.0   # E -> PV spread
    pv_out_sigma: float = 1.0  # PV -> E spread
...
class RgcLgnV1Network:
    ...
    def __init__(self, p: Params, *, init_mode: str = "random"):
        ...
        # For grating stimuli, an isotropic DoG acts as an orientation-independent scalar gain in the
        # infinite-plane limit. The padded_fft implementation filters on a regular grid and then samples
        # to the (possibly jittered) mosaic, which can introduce small orientation-dependent attenuation
        # on small patches. To avoid systematic orientation biases during Phase-A grating rearing, we
        # precompute a single scalar gain that matches the *mean* padded_fft response.
        self._rgc_grating_dog_gain_on: float | None = None
        self._rgc_grating_dog_gain_off: float | None = None
        ...
        # Optionally include a small per-ensemble RF-center jitter. This is biologically plausible
        # (RF centers are not perfectly identical within a local neighborhood) and prevents a
        # pathological symmetry where many ensembles converge to the same feature under purely
        # feedforward Hebbian learning.
        self.tc_center_dx = np.zeros(p.M, dtype=np.float32)
        self.tc_center_dy = np.zeros(p.M, dtype=np.float32)
        if float(p.tc_center_jitter) > 0.0:
            j = float(p.tc_center_jitter)
            rng_cent = np.random.default_rng(np.random.SeedSequence([p.seed, 424242, 0]))
            self.tc_center_dx = rng_cent.uniform(-j, j, size=p.M).astype(np.float32)
            self.tc_center_dy = rng_cent.uniform(-j, j, size=p.M).astype(np.float32)
            # Keep the global patch centered (avoid introducing a net shift).
            self.tc_center_dx -= float(self.tc_center_dx.mean())
            self.tc_center_dy -= float(self.tc_center_dy.mean())

        def lgn_mask_mat(sigma: float, post_dx: np.ndarray, post_dy: np.ndarray) -> np.ndarray:
            """Retinotopic cap matrix for a set of postsynaptic units with center offsets.

            Parameters
            ----------
            sigma : float
                Gaussian sigma in pixels; <=0 disables the cap (all ones).
            post_dx, post_dy : ndarray, shape (n_post,)
                Retinotopic center offsets in pixels for each postsynaptic unit.

            Returns
            -------
            ndarray, shape (n_post, n_lgn)
                Values in [0,1] (per-row max normalized).
            """
            sigma = float(sigma)
            if sigma <= 0.0:
                return np.ones((int(post_dx.shape[0]), self.n_lgn), dtype=np.float32)
            post_dx = np.asarray(post_dx, dtype=np.float32).reshape(-1)
            post_dy = np.asarray(post_dy, dtype=np.float32).reshape(-1)
            x_on = self.X_on.ravel().astype(np.float32, copy=False)
            y_on = self.Y_on.ravel().astype(np.float32, copy=False)
            x_off = self.X_off.ravel().astype(np.float32, copy=False)
            y_off = self.Y_off.ravel().astype(np.float32, copy=False)
            dx = post_dx[:, None]
            dy = post_dy[:, None]
            d2_on = (x_on[None, :] - dx) ** 2 + (y_on[None, :] - dy) ** 2
            d2_off = (x_off[None, :] - dx) ** 2 + (y_off[None, :] - dy) ** 2
            inv2s2 = float(1.0 / (2.0 * sigma * sigma))
            pix_on = np.exp(-d2_on * inv2s2).astype(np.float32, copy=False)
            pix_off = np.exp(-d2_off * inv2s2).astype(np.float32, copy=False)
            vec = np.concatenate([pix_on, pix_off], axis=1).astype(np.float32, copy=False)
            vec /= vec.max(axis=1, keepdims=True) + 1e-12
            return vec

        if float(p.tc_center_jitter) > 0.0:
            pv_parent = (np.arange(self.n_pv, dtype=np.int32) // max(1, int(p.n_pv_per_ensemble))).astype(
                np.int32, copy=False
            )
            pp_parent = (np.arange(self.n_pp, dtype=np.int32) // max(1, int(p.n_pp_per_ensemble))).astype(
                np.int32, copy=False
            )
            self.lgn_mask_e = lgn_mask_mat(p.lgn_sigma_e, self.tc_center_dx, self.tc_center_dy)
            self.lgn_mask_pv = lgn_mask_mat(p.lgn_sigma_pv, self.tc_center_dx[pv_parent], self.tc_center_dy[pv_parent])
            self.lgn_mask_pp = lgn_mask_mat(p.lgn_sigma_pp, self.tc_center_dx[pp_parent], self.tc_center_dy[pp_parent])
        else:
        ...
        def sample_tc_mask(n_post: int, frac: float, mask_w: np.ndarray, seed_tag: int) -> np.ndarray:
            frac = float(frac)
            if not (0.0 < frac <= 1.0):
            ...
            if mask_w.ndim == 1:
                mask_w = np.tile(mask_w[None, :], (n_post, 1))
            elif mask_w.shape == (1, self.n_lgn) and n_post > 1:
                mask_w = np.tile(mask_w, (n_post, 1))
            if mask_w.shape != (n_post, self.n_lgn):
                raise ValueError(
                    f"mask_w must have shape ({n_post},{self.n_lgn}) or ({self.n_lgn},), got {tuple(mask_w.shape)}"
                )
            ...
            if p.tc_conn_balance_onoff:
                ...
                for i in range(n_post):
                    if n_on > 0:
                        prob_on = mask_w[i, :n_pix].astype(np.float64, copy=True)
                        prob_on /= float(prob_on.sum() + 1e-12)
                        on_idx = tc_rng.choice(n_pix, size=n_on, replace=False, p=prob_on)
                        mask[i, on_idx] = True
                    if n_off > 0:
                        prob_off = mask_w[i, n_pix:].astype(np.float64, copy=True)
                        prob_off /= float(prob_off.sum() + 1e-12)
                        off_idx = tc_rng.choice(n_pix, size=n_off, replace=False, p=prob_off)
                        mask[i, n_pix + off_idx] = True
            else:
                for i in range(n_post):
                    prob = mask_w[i].astype(np.float64, copy=True)
        ...
        self.tc_mask_e = sample_tc_mask(p.M, p.tc_conn_fraction_e, self.lgn_mask_e, seed_tag=1)
        self.tc_mask_pv = sample_tc_mask(self.n_pv, p.tc_conn_fraction_pv, self.lgn_mask_pv, seed_tag=2)
        self.tc_mask_pp = sample_tc_mask(self.n_pp, p.tc_conn_fraction_pp, self.lgn_mask_pp, seed_tag=3)
    ...
    def _setup_rgc_dog_padded_fft(self) -> None:
        ...
        # Precompute an orientation-averaged grating gain for the padded_fft DoG path.
        # This is a deterministic calibration (seeded) and does not affect the main RNG stream.
        thetas = np.linspace(0.0, 180.0, 12, endpoint=False, dtype=np.float64)
        rng_cal = np.random.default_rng(np.random.SeedSequence([p.seed, 33333, 0]))
        phases = rng_cal.uniform(0.0, 2.0 * math.pi, size=3).astype(np.float64)
        gains_on: list[float] = []
        gains_off: list[float] = []
        for ph in phases:
            for th in thetas:
                stim_pad = self.grating_on_coords(float(th), t_ms=0.0, phase=float(ph), X=self._X_pad, Y=self._Y_pad)
                drive_on, drive_off = self._rgc_drives_from_pad_stimulus(stim_pad, contrast=1.0)
                raw_on = self.grating_on_coords(float(th), t_ms=0.0, phase=float(ph), X=self.X_on, Y=self.Y_on)
                raw_off = self.grating_on_coords(float(th), t_ms=0.0, phase=float(ph), X=self.X_off, Y=self.Y_off)

                x_on = raw_on.ravel().astype(np.float64, copy=False)
                y_on = drive_on.ravel().astype(np.float64, copy=False)
                denom_on = float(x_on @ x_on) + 1e-12
                gains_on.append(float((x_on @ y_on) / denom_on))

                x_off = raw_off.ravel().astype(np.float64, copy=False)
                y_off = drive_off.ravel().astype(np.float64, copy=False)
                denom_off = float(x_off @ x_off) + 1e-12
                gains_off.append(float((x_off @ y_off) / denom_off))

        self._rgc_grating_dog_gain_on = float(np.mean(np.asarray(gains_on, dtype=np.float64)))
        self._rgc_grating_dog_gain_off = float(np.mean(np.asarray(gains_off, dtype=np.float64)))
    ...
    def rgc_drives_grating(
        ...
        *,
        ...
        if (self._rgc_grating_dog_gain_on is not None) and (self._rgc_grating_dog_gain_off is not None):
            raw_on = self.grating_on_coords(theta_deg, t_ms, phase, self.X_on, self.Y_on)
            raw_off = self.grating_on_coords(theta_deg, t_ms, phase, self.X_off, self.Y_off)
            drive_on = (contrast * float(self._rgc_grating_dog_gain_on) * raw_on).astype(np.float32, copy=False)
            drive_off = (contrast * float(self._rgc_grating_dog_gain_off) * raw_off).astype(np.float32, copy=False)
            return drive_on, drive_off
    ...
    def measure_drive_fraction(
        ...
        *,
        ...
        saved = {
            ...
            'pp_v': self.pp.v.copy(), 'pp_u': self.pp.u.copy(),
            ...
            'g_v1_inh_pp': self.g_v1_inh_pp.copy(),
            'I_lgn': self.I_lgn.copy(), 'I_pv': self.I_pv.copy(),
            'I_pv_inh': self.I_pv_inh.copy(),
            'I_pp': self.I_pp.copy(), 'I_som': self.I_som.copy(),
        ...
        self.pp.v = saved['pp_v']; self.pp.u = saved['pp_u']
        ...
        self.g_v1_inh_pp = saved['g_v1_inh_pp']
        self.I_lgn = saved['I_lgn']; self.I_pv = saved['I_pv']
        self.I_pv_inh = saved['I_pv_inh']
        self.I_pp = saved['I_pp']; self.I_som = saved['I_som']
    ...
    def save_dynamic_state(self) -> dict:
        ...
        s["pp_v"] = self.pp.v.copy()
        s["pp_u"] = self.pp.u.copy()
        ...
        s["g_v1_inh_pp"] = self.g_v1_inh_pp.copy()
        s["I_pv"] = self.I_pv.copy()
        s["I_pv_inh"] = self.I_pv_inh.copy()
        s["I_pp"] = self.I_pp.copy()
    ...
    def restore_dynamic_state(self, s: dict) -> None:
        ...
        self.g_v1_inh_pp = s["g_v1_inh_pp"]
        self.I_pv = s["I_pv"]
...
def plot_ee_weight_matrix_evolution(
    ...
    """Plot W_e_e heatmaps at training checkpoints, sorted by preferred orientation.

    1\u00d74 subplots, neurons sorted by pref on both axes, shared color scale,
    colored tick marks for sequence element groups.
    """
    checkpoints = sorted(W_snapshots.keys())
    n_ckpt = len(checkpoints)
    fig, axes = plt.subplots(1, n_ckpt, figsize=(4 * n_ckpt, 4))
    if n_ckpt == 1:
        axes = [axes]

    sort_idx = np.argsort(pref_shared)
    pref_sorted = pref_shared[sort_idx]

    # Shared color range
    vmin = min(float(W_snapshots[c][np.ix_(sort_idx, sort_idx)].min()) for c in checkpoints)
    vmax = max(float(W_snapshots[c][np.ix_(sort_idx, sort_idx)].max()) for c in checkpoints)

    seq_colors = ["#e41a1c", "#377eb8", "#4daf4a", "#984ea3"]

    for i, ckpt in enumerate(checkpoints):
        ax = axes[i]
        W_sorted = W_snapshots[ckpt][np.ix_(sort_idx, sort_idx)]
        im = ax.imshow(W_sorted, aspect="equal", cmap="viridis",
                       vmin=vmin, vmax=vmax, interpolation="nearest")
        ax.set_title(f"After {ckpt} pres.", fontsize=9)
        ax.set_xlabel("Pre neuron (by pref)")
        if i == 0:
            ax.set_ylabel("Post neuron (by pref)")

        # Colored markers for sequence element groups
        for j, theta in enumerate(seq_thetas):
            d = np.abs(pref_sorted - theta)
            d = np.minimum(d, 180.0 - d)
            members = np.where(d < 22.5)[0]
            for m in members:
                ax.plot(-0.8, m, "s", color=seq_colors[j % len(seq_colors)],
                        markersize=3, clip_on=False)

        # Arrow annotation showing sequence flow direction
        M = len(pref_sorted)
        if i == n_ckpt - 1:
            ax.annotate("", xy=(M * 0.85, M * 0.15), xytext=(M * 0.15, M * 0.85),
                        arrowprops=dict(arrowstyle="->", color="white", lw=1.5))
            ax.text(M * 0.55, M * 0.45, "seq\u2192", color="white", fontsize=8,
                    ha="center", va="center", rotation=-45)

    fig.colorbar(im, ax=list(axes), shrink=0.8, label="Weight")
...
def plot_omission_prediction_growth(
    ...
    for ci, ckpt in enumerate(sorted_ckpts):
        ...
        if ci == 0:
            ...
            pvals.append(wilcoxon_signed_rank_greater(adjusted))
        else:
            pvals.append(1.0)
...
def plot_omission_activity_traces(
    ...
    Panel A: Normalized baseline g_exc_ee traces (pres=0) — equalized by
             per-source-neuron normalization, confirming fair comparison.
    ...
    ax.plot(t_ms, om_base, "-", color="#2166ac", linewidth=1.2,
            label="Trained context (0\u00b0 \u2192 BLANK)", alpha=0.9)
    ax.plot(t_ms, ct_base, "-", color="#d6604d", linewidth=1.2,
            label="Control context (90\u00b0 \u2192 BLANK)", alpha=0.9)
    ...
    ax.set_ylabel("g_exc_ee / n_source")
    ax.set_title("A. Baseline (0 presentations): normalized per source neuron",
    ...
    for ei in range(1, n_elements + 1):
        ax.axvline(ei * element_ms, color="gray", linestyle="--", linewidth=0.5, alpha=0.5)
    ax.set_ylabel("\u0394 g_exc_ee / n_source")
...
def run_sequence_experiment(args) -> List[dict]:
    ...
    p_kwargs: dict = dict(
        ...
        # Separate ON/OFF mosaics are biologically realistic and help prevent
        # systematic orientation-collapse under pure feedforward STDP.
...
def run_self_tests(out_dir: str) -> None:
    ...
    t_shared_p = Params(
        ...
        train_contrast=2.0,
        v1_bias_init=0.0,
        # Separate ON/OFF mosaics reduce lattice-induced orientation collapse in Phase A.
        rgc_separate_onoff_mosaics=True,
        ee_stdp_enabled=True,
        ee_connectivity="all_to_all",
        ee_stdp_A_plus=0.005,
    ...
    t30_seq_thetas = [0.0, 45.0, 90.0, 135.0]
    t_shared_net.ff_plastic_enabled = False
    t_shared_net.ee_stdp_active = True
    t_shared_net._ee_stdp_ramp_factor = 1.0

    # Identify neuron groups for omission prediction
    t31_omit_index = 1  # omit 2nd element (45°)
    omit_theta = t30_seq_thetas[t31_omit_index]           # 45°
    pre_omit_theta = t30_seq_thetas[t31_omit_index - 1]   # 0°
    ctrl_pre_theta = float((pre_omit_theta + 90.0) % 180.0)  # 90°

    d_omit = np.abs(pref_shared - omit_theta)
    d_omit = np.minimum(d_omit, 180.0 - d_omit)
    omit_mask = d_omit < 22.5
    ...
    def _sim_prediction():
        ...
        t_shared_net.reset_state()
        om = run_sequence_trial(t_shared_net, t30_seq_thetas, 30.0, 200.0, 2.0,
                                plastic=False, omit_index=oi,
                                record=True, vep_mode="g_exc_ee")
        t_shared_net.reset_state()
        oc = run_sequence_trial(t_shared_net, ctrl_thetas_31, 30.0, 200.0, 2.0,
                                plastic=False, omit_index=oi,
                                record=True, vep_mode="g_exc_ee")
    ...
    def _fwd_rev_asymmetry_30(net30, seq_th, pref_deg):
        ...
        for ei in range(len(seq_th) - 1):
            ...
            for pi in np.where(d_post < 22.5)[0]:
                for pj in np.where(d_pre_a < 22.5)[0]:
                    if pi != pj:
    ...
    def _collect_traces(n_avg_trace=20, n_avg_spk=20):
        ...
        oi = t31_omit_index
        dt = t_shared_p.dt_ms
        omit_start_step = int(oi * 30.0 / dt)
        omit_end_step = int((oi + 1) * 30.0 / dt)
        ...
        for _ in range(n_avg_trace):
            t_shared_net.vep_target_mask = omit_mask
            t_shared_net.reset_state()
            om_res = run_sequence_trial(t_shared_net, t30_seq_thetas, 30.0, 200.0, 2.0,
                                        plastic=False, omit_index=oi,
                                        record=True, vep_mode="g_exc_ee")
            om_full_raw = np.concatenate(om_res["element_traces"] + [om_res["iti_trace"]])
            om_traces.append(om_full_raw / n_pre)   # normalized for visualization

            t_shared_net.reset_state()
            ct_res = run_sequence_trial(t_shared_net, ctrl_thetas_31, 30.0, 200.0, 2.0,
                                        plastic=False, omit_index=oi,
                                        record=True, vep_mode="g_exc_ee")
            ct_full_raw = np.concatenate(ct_res["element_traces"] + [ct_res["iti_trace"]])
            ct_traces.append(ct_full_raw / n_ctrl)   # normalized for visualization
            ...
            t_shared_net.vep_target_mask = None
            t_shared_net.reset_state()
            fs_res = run_sequence_trial(t_shared_net, t30_seq_thetas, 30.0, 200.0, 2.0,
    ...
    for k in range(1, t30_total + 1):
        run_sequence_trial(t_shared_net, t30_seq_thetas, 30.0, 200.0, 2.0,
                           plastic=True, vep_mode="spikes")
        if k % 25 == 0:
    ...
    for ci in range(2, len(t30_ratios)):
        assert t30_ratios[ci] >= t30_ratios[ci - 1] * 0.90, (
            f"CRITERION 2 FAILED: ratio dropped at checkpoint {ci}: "
            f"{t30_ratios[ci]:.3f} < {t30_ratios[ci-1]:.3f}*0.90")
    ...
    plot_omission_activity_traces(
        ...
        t30_diff_trials,
        30.0, 200.0, t30_seq_thetas, t31_omit_index, t_shared_p.dt_ms,
    ...
    plot_full_sequence_response_evolution(
        t30_full_seq_traces, 30.0, 200.0, t30_seq_thetas, t_shared_p.dt_ms,
...
def main() -> None:
    ...
    ap.add_argument("--tc-center-jitter", type=float, default=None,
                    help="per-ensemble retinotopic center jitter for thalamocortical caps/masks (pixels; 0 disables)")
    ...
    if args.tc_center_jitter is not None:
        p_kwargs["tc_center_jitter"] = float(args.tc_center_jitter)
    ...
    if float(getattr(p, "tc_center_jitter", 0.0)) > 0.0:
        print(f"[init] Thalamocortical RF-center jitter: {float(p.tc_center_jitter):.2f} px")
